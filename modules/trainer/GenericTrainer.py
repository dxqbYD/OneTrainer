        ...
        for _epoch in tqdm(range(train_progress.epoch, self.config.epochs, 1), desc="epoch"):
            ...
            for batch in step_tqdm:
                ...
                with TorchMemoryRecorder(enabled=False):
                    ...
                    #loss = loss_w
                    #loss_diff = loss_w - loss_l

                    #assert torch.equal(model_output_data['target'][0],model_output_data['target'][1])
                    #model_output_data['target'][0] = model_output_data['predicted'][1] + ref_output_data['predicted'][0] - ref_output_data['predicted'][1]
                    #model_output_data['target'][1] = model_output_data['predicted'][1] # not necessary
                    #loss = self.model_setup.calculate_loss(self.model, batch, model_output_data, self.config, reduce=False)


                    #loss = loss[0]
                    #print(loss)
                    #positive_loss = 0.693 * torch.log(1 + loss[0])
                    #negative_loss = - torch.nn.functional.logsigmoid(loss[1])
                    #print(f"{positive_loss} vs. {negative_loss}")
                    ...
                    #inside_term = - beta_dpo * (loss_diff - ref_loss_diff)
                    #loss = - torch.nn.functional.logsigmoid(inside_term)
                    ##print(f"inside_term: {inside_term}  loss_diff: {loss_diff}  ref_loss_diff: {ref_loss_diff}")
                    ##self.tensorboard.add_scalar("loss/diff", diff_w - diff_l, train_progress.global_step)
                    ...
                    ##self.tensorboard.add_scalar("loss/model_loss_diff", loss_w - loss_l, train_progress.global_step)
                    ##self.tensorboard.add_scalar("loss/ref_loss_diff", ref_loss_w - ref_loss_l, train_progress.global_step)
                    ...
                    def print_grad_norm():
                        for param in self.model.parameters.parameters():
